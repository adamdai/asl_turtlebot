...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        # Ensure that V doesn't make J singular
        if V_prev < V_PREV_THRES:
            V_prev = np.sqrt(xd_d**2 + yd_d**2)

        # Calculate x_dd and y_dd
        xdd = xdd_d - self.kpx * (x - x_d) - self.kdx * (V_prev * np.cos(th) - xd_d)
        ydd = ydd_d - self.kpy * (y - y_d) - self.kdy * (V_prev * np.sin(th) - yd_d)

        # Define J
        J = np.array([[np.cos(th), -V_prev*np.sin(th)], [np.sin(th), V_prev*np.cos(th)]])

        # Invert J to find a and om
        a, om = np.linalg.solve(J, np.array([xdd, ydd]))

        # Solve for V after one time step
        V = V_prev + a * dt
